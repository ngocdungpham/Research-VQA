...
DEFAULT_OUTPUT_DIR = ROOT / "canonical_v2_2"
DEFAULT_SCHEMA = SCRIPT_DIR / "canonical_schema_v2_1.json"
DEFAULT_TAXONOMY = SCRIPT_DIR / "bronchoscopy_taxonomy_v2.json"
DATASET_VERSION = "canonical_v2.2"
SCHEMA_VERSION = "bronchoscopy_canonical_schema_v2.1"
...
def make_regions(
    ...
    for finding_index, finding in enumerate(findings):
        ...
        if not source_annotation_id:
            source_annotation_id = "synthetic:" + stable_id(
                "annotation", source_path, source_image_id, str(finding_index), key, length=24
            )
            evidence_flags.add("missing_source_annotation_id_synthetic_id_assigned")
        group["source_annotation_ids"].add(source_annotation_id)
        evidence_labels = empty_region_labels(taxonomy)
        evidence_label_ids: set[str] = set()
        evidence_unmapped: list[dict[str, Any]] = []
        for label_id in finding.get("label_ids", []) or []:
            label_id = str(label_id)
            group["source_label_ids"].add(label_id)
            evidence_label_ids.add(label_id)
            name, resolution_flags = resolve_label(label_id, catalog_name, catalogs)
            evidence_flags.update(resolution_flags)
            if source_name := name:
                mapped = apply_region_label(evidence_labels, source_name, taxonomy, evidence_flags)
            else:
                mapped = False
            if not mapped:
                candidate = {"id": label_id, "name": name}
                if candidate not in evidence_unmapped:
                    evidence_unmapped.append(candidate)
        ...
        group["source_evidence"].append(
            {
                ...
                "source_label_ids": sorted(evidence_label_ids),
                "canonical_labels": evidence_labels,
                "source_geometry_hash": key,
                "unmapped_source_labels": sorted(
                    evidence_unmapped, key=lambda row: (row["id"], row["name"] or "")
                ),
                "quality_flags": sorted(evidence_flags),
            }
        )
    ...
    for index, key in enumerate(sorted(grouped), start=1):
        ...
        for evidence in group["source_evidence"]:
            flags.update(evidence["quality_flags"])
            for candidate in evidence["unmapped_source_labels"]:
                if candidate not in unmapped:
                    unmapped.append(candidate)
                counters["unmapped_region_label_occurrences"] += 1
            for label_id in evidence["source_label_ids"]:
                source_name, resolution_flags = resolve_label(label_id, catalog_name, catalogs)
                flags.update(resolution_flags)
                if source_name is not None:
                    apply_region_label(labels, source_name, taxonomy, flags)
        ...
        region = {
            ...
            "source_evidence": sorted(
                group["source_evidence"], key=lambda row: row["source_annotation_id"]
            ),
...
def build(args: argparse.Namespace) -> None:
    ...
    quality_report = {
        ...
        "notes": [
            ...
            " Each source_evidence entry retains the exact annotation-to-label association."
        ],
    }
